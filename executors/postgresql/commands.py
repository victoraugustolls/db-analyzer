...
class IndexCommand(Command):
    ...
    _update_queries: list[Query]
    _hypo_index_id: int | None = None
    _create_cost: float | None = None
    _update_cost: float | None = None

    def __init__(self, suggestion: Suggestion, queries: list[Query], conn: asyncpg.pool.Pool):
        ...
        self._update_queries = list(filter(
            lambda x: x.table == self._extract_table_name_from_query(suggestion.action.command),
            queries,
        ))
    ...
    async def _get_update_cost(self) -> float:
        ...
        self._update_cost = record * len(self._update_queries)
        return self._update_cost
